...
Our 95% confidence interval using prop.test (R's built-in method) for the difference in proportions of those who voted and did versus did not receive a call is between 0.141 to 0.266. Since this interval does not include 0, it is statistically significant. This means that there is a relationship between receiving a GOTV call and voting in the 1998 election.
...
As is evident by the presentation of summary statistics, there is a clear difference in proportions for those who received a GOTV call and those who did not. For instance, the mean age of those who received a called was around 58, but those who did not had a mean age of only 49. Since older people have a higher propensity to vote, this is a confounding variable. This observation remains true for the voted in 1996 and major party variables as well. Since these variables affect the treatment and the outcome variable, they are all confounders.
...
Additionally, our 95% confidence interval for the difference in mean ages between those who did not receive a GOTV_call and those who did is between -11.40 to -6.37 which is statistically significant since it does not include zero, further supporting the idea that age is a confounder in this case.
...
The 95% confidence interval here is [0.122, 0.241]. Since this interval does not include 0, it supports the conclusion that voting in 1996 is a confounding variable.
...
summary_stats
```
The above histogram depicts the similarities in our age variable and distribution after matching. This shows almost zero difference between the two due to our matching.
### Age (Matched)
```{r echo=FALSE}
...
```
The 95% confidence interval for the difference in proportions of those who voted in 1996 and did/did not receive GOTV_call to illustrate this difference ranged from -0.03 to 0.10 meaning it is not statistically significant, further supporting the claim that GOTV_call is not correlated to being registered for a major party in our matched data set.
### Major Party (Matched)
...
The 95% confidence interval for the difference in proportions of those who were registered for a major party and did/did not receive GOTV_call to illustrate this difference ranged from -0.05 to 0.07 meaning it is not statistically significant, further supporting the claim that GOTV_call is not correlated to being registered for a major party in our matched data set.
...
With the matched data set, we can clearly see that those who received the call tended to vote more than those who did not.
